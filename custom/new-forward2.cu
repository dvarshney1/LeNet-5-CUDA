...
__constant__ float k[K_COMMON * K_COMMON * C_LAYER2 * M_LAYER2];
...
__host__ void GPUInterface::conv_forward_gpu2(float *host_y, const float *host_x, const float *host_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    const int H_out = H - K + 1;
    const int W_out = W - K + 1;
    ...
    cudaMemcpyToSymbol(k, host_k, k_len * sizeof(float));
