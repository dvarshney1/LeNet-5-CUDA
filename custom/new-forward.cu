...
#define K_COMMON 7
#define K_COMMON_RADIUS 3
#define H_OUT_LAYER1 80
#define W_OUT_LAYER1 80
#define H_OUT_LAYER2 34
#define W_OUT_LAYER2 34
#define H_GRID_LAYER1 5
#define W_GRID_LAYER1 5
#define H_GRID_LAYER2 5
#define W_GRID_LAYER2 5
#define C_LAYER1 1
#define M_LAYER1 4
#define C_LAYER2 4
#define M_LAYER2 16
...
#define y4d(i3, i2, i1, i0) y[(i3) * (M * H_OUT_LAYER1 * W_OUT_LAYER1) + (i2) * (H_OUT_LAYER1 * W_OUT_LAYER1) + (i1) * (W_OUT_LAYER1) + i0]
...
#define loc_mem(i2, i1, i0) loc_mem1[(i2) * ((TILE_WIDTH+K_COMMON-1) * (TILE_WIDTH+K_COMMON-1)) + (i1) * (TILE_WIDTH+K_COMMON-1) + (i0)]
    ...
    b = blockIdx.x;
    m = blockIdx.y;
    h = (blockIdx.z/W_GRID_LAYER1)*TILE_WIDTH + threadIdx.y;
    w = (blockIdx.z % W_GRID_LAYER1)*TILE_WIDTH + threadIdx.x;
    ...
    if (h < H_OUT_LAYER1 && w < W_OUT_LAYER1){
      half acc = 0.0f;
      #pragma unroll 7
      for (int p = 0; p < K; p++) {
          #pragma unroll 7
...
#define y4d(i3, i2, i1, i0) y[(i3) * (M * H_OUT_LAYER2 * W_OUT_LAYER2) + (i2) * (H_OUT_LAYER2 * W_OUT_LAYER2) + (i1) * (W_OUT_LAYER2) + i0]
#define x4d(i3, i2, i1, i0) x[(i3) * (C * H * W) + (i2) * (H * W) + (i1) * (W) + i0]
...
#define loc_mem(i2, i1, i0) loc_mem2[(i2) * ((TILE_WIDTH_2+K-1) * (TILE_WIDTH_2+K-1)) + (i1) * (TILE_WIDTH_2+K-1) + (i0)]
    ...
    b = blockIdx.x;
    m = blockIdx.y;
    h = (blockIdx.z/W_GRID_LAYER2)*TILE_WIDTH_2 + threadIdx.y;
    w = (blockIdx.z % W_GRID_LAYER2)*TILE_WIDTH_2 + threadIdx.x;

    __shared__ half loc_mem2[C_LAYER2 * (TILE_WIDTH_2 + K_COMMON - 1) * (TILE_WIDTH_2 + K_COMMON - 1)]; // set size on launch
    ...
    if (h < H_OUT_LAYER2 && w < W_OUT_LAYER2) {
        half acc = 0.0f;
        #pragma unroll 4 //6
        for (int c = 0; c < C; c++) {
            #pragma unroll 7
...
__host__ void GPUInterface::conv_forward_gpu(float *host_y, const float *host_x, const float *host_k, const int B,
                                            const int M, const int C, const int H, const int W, const int K)
{
    ...
    if(C != 1) tile_width = TILE_WIDTH_2;
    ...
    if (C == 1) { conv_forward_kernel_layer1<<<DimGrid, DimBlock>>> (device_y, device_x, B, M, C, H, W, K);}
    else { conv_forward_kernel_layer2<<<DimGrid, DimBlock>>> (device_y, device_x, B, M, C, H, W, K); }
